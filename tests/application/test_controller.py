...
from jotta_gui.application.state import SyncState
...
@pytest.mark.parametrize(
    ("method_name", "command_name", "expected_state"),
    [
        ("start_sync", "sync_start", SyncState.STARTING),
        ("stop_sync", "sync_stop", SyncState.STOPPING),
        ("trigger_sync", "sync_trigger", SyncState.SYNCING),
    ],
)
def test_sync_action_sets_pending_state_and_runs_command(
    qt_app,
    monkeypatch,
    method_name: str,
    command_name: str,
    ...
    assert controller.state.sync_state == expected_state
    assert controller.state.error_message is None
    assert calls == [controller.runner]


def test_status_output_updates_state_and_requests_runtime_status(
    qt_app,
    monkeypatch,
    status_output: str,
) -> None:
    ...
    assert controller.state.sync_state == SyncState.UNKNOWN
    assert controller.state.error_message is None
...
def test_runtime_output_maps_active_state(
    qt_app,
    monkeypatch,
    status_output: str,
) -> None:
    ...
    assert controller.state.sync_state == SyncState.ACTIVE


def test_runtime_error_preserves_connection(qt_app) -> None:
    controller = ApplicationController()

from __future__ import annotations

import json
from pathlib import Path

import pytest

from jotta_gui.devtools.fixtures import (
    Capture,
    SANITIZED_SYNC_ROOT,
    sanitize_capture,
    sanitize_status,
    write_capture,
)


def _payload() -> dict:
    return {
        "User": {
            "Email": "private@example.org",
            "Fullname": "Private Person",
            "Hostname": "private-host",
        },
        "Sync": {"RootPath": "/home/private/Jotta"},
        "Backup": {
            "State": {
                "Enabled": {
                    "Backups": [
                        {
                            "Name": "Secret Documents",
                            "Path": "/home/private/Documents",
                            "DeviceID": "real-device-id",
                        }
                    ]
                }
            }
        },
    }


def test_sanitize_status_replaces_personal_fields() -> None:
    sanitized, replacements = sanitize_status(_payload())

    user = sanitized["User"]
    sync = sanitized["Sync"]
    backup = sanitized["Backup"]["State"]["Enabled"]["Backups"][0]

    assert user["Email"] == "user@example.com"
    assert user["Fullname"] == "Example User"
    assert user["Hostname"] == "example-host"
    assert sync["RootPath"] == SANITIZED_SYNC_ROOT
    assert backup["Name"] == "Backup 1"
    assert backup["Path"] == "/home/example/backup-1"
    assert backup["DeviceID"] == "device-1"
    assert replacements["/home/private/Jotta"] == SANITIZED_SYNC_ROOT


def test_sanitize_capture_keeps_runtime_root_in_sync() -> None:
    capture = Capture(
        status=_payload(),
        runtime="Path: /home/private/Jotta\nMode: listening to events\n",
        replacements={},
    )

    sanitized = sanitize_capture(capture)

    assert f"Path: {SANITIZED_SYNC_ROOT}" in sanitized.runtime
    assert "/home/private/Jotta" not in sanitized.runtime


def test_sanitize_capture_redacts_unexpected_runtime_email() -> None:
    capture = Capture(
        status=_payload(),
        runtime="Contact: other.person@private.example\n",
        replacements={},
    )

    sanitized = sanitize_capture(capture)

    assert "other.person@private.example" not in sanitized.runtime
    assert "user@example.com" in sanitized.runtime


def test_write_capture_creates_schema_two_metadata(tmp_path: Path) -> None:
    capture = sanitize_capture(
        Capture(
            status=_payload(),
            runtime="Path: /home/private/Jotta\nMode: listening to events\n",
            replacements={},
        )
    )

    destination = write_capture(
        "Linux Active",
        capture,
        scenario="automatic-listening",
        expected_sync_mode="automatic",
        expected_runtime_state="listening",
        output_root=tmp_path,
    )

    metadata = json.loads((destination / "metadata.json").read_text())
    assert (destination / "status.json").is_file()
    assert (destination / "runtime.txt").is_file()
    assert metadata["schema"] == 2
    assert metadata["expected_sync_mode"] == "automatic"
    assert metadata["expected_runtime_state"] == "listening"


def test_write_capture_rejects_unsafe_name(tmp_path: Path) -> None:
    capture = Capture(status=_payload(), runtime="", replacements={})

    with pytest.raises(ValueError):
        write_capture("../outside", capture, output_root=tmp_path)
